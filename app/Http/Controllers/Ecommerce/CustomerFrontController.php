...
class CustomerFrontController extends Controller
{
    ...
    public function customer_login(Request $request)
    {
        ...
        //     'is_ios'          => (bool) preg_match('/(iPhone|iPod|iPad)/i', $request->userAgent())
        // ]);

        // Check if source parameter is passed as 'ios'
        $isIosSource = strtolower($request->input('source')) === 'ios';

        // Check if User-Agent header strictly identifies an iOS device
        $isIosDevice = (bool) preg_match('/(iPhone|iPod|iPad)/i', $request->userAgent());

        // If source is specified as iOS but the device/User-Agent isn't iOS, block and redirect
        if ($isIosSource) {
            if (!$isIosDevice) {
