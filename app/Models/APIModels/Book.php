...
class Book extends Model
{
  ...
  public function getBookList($data){
   ...
   // $UserID=$data['UserID'];
    ...
    $list = Cache::remember($CacheKey, now()->addHours(7), function () use ($UserID, $Status, $SearchText, $Limit, $PageNo) {
...
public function getRandomBookList($data){
    ...
    // NOTE: intentionally NOT cached — ORDER BY RAND() caching would freeze
    // the "random" result for 7 hours, which changes the intended behavior.
    // Let me know if you want this cached with a short TTL instead.
...
public function getContinueToReadBookList($data){
    ...
    $list = Cache::remember($CacheKey, now()->addHours(7), function () use ($UserID) {
...
public function getSearchBookList($data){
    ...
    $list = Cache::remember($CacheKey, now()->addHours(7), function () use ($UserID, $Status, $SearchText, $Filter_Sort, $Filter_Genre, $Filter_Star) {
...
public function getSearchAudioBookList($data){
    ...
    $list = Cache::remember($CacheKey, now()->addHours(7), function () use ($UserID, $Status, $SearchText) {
