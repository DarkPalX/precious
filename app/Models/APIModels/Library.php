...
class Library extends Model
{
 ...
 public function getLibraryList($data){
    ...
    $list = Cache::remember($CacheKey, now()->addSeconds(40), function () use ($UserID, $SearchText) {
        ...
        $query = DB::table('customer_libraries as lib')
            ...
            ->selectraw("
                ...
                COALESCE(prds.ebook_price - (promo.promo_discount_percent / 100 * prds.ebook_price),0) as promo_discount_price,
...
public function getSubscribedReadBooksList($data){
    ...
    $list = Cache::remember($CacheKey, now()->addMinutes(3), function () use ($UserID, $SearchText) {
        ...
        $query = DB::table('subscribed_books as rbooks')
            ...
            ->leftJoin('vw_product_primary_image as img', 'img.product_id', '=', 'rbooks.product_id')
            ->leftJoin('vw_product_rating as rt', 'rt.product_id', '=', 'prds.id')

            ->leftJoin('vw_product_active_promo as promo', 'promo.product_id', '=', 'rbooks.product_id')
