...
<div style="margin: 40px 40px 200px 40px;font-family:Arial;">
    <h4 class="mg-b-0 tx-spacing--1">Downloaders of {{ $product->name ?? '' }}</h4>
    
    {{-- <form action="{{route('report.customer-downloads.mobile')}}" method="get">
    ...
    <table id="example" class="display nowrap" style="width:100%;font: normal 13px/150% Arial, sans-serif, Helvetica;">
        ...
        <tbody>
            @forelse($rs as $r)
            <tr>
                <td>{{ \App\Models\User::getUser($r->user_id)?->name ?? 'Deleted User' }}</td>
            </tr>
            @empty
            <tr>
